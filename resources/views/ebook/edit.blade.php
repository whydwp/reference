        ...
        <form class="form-horizontal" method="post" action="{{ route('ebook.update',[$ebook->id_ebook]) }}" enctype="multipart/form-data">
            ...
            <div class="embed-responsive embed-responsive-4by3">
                    <iframe class="embed-responsive-item" type="application / html5"
                        src="{{ asset('uploads/'.$ebook->file) }}#toolbar=0&navpanes=0&scrollbar=0" width="100%" height="10000">{{ asset('uploads/'.$ebook->file) }}</iframe>
            </div>
            <br>
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <select id="id_status" name="id_status" class="form-control @error('id_status') is-invalid @enderror">      
                            @foreach($status as $row )
                            <option value="{{ $row->id_status }}" 
                                @if($ebook->id_status == $row->id_status) Selected 
                                @endif>{{ $row->status }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div class="form-group">
                ...
                <textarea id="deskripsi_dokumen" name="deskripsi_ebook"
                    ...
                    rows="10" disabled>{{$ebook->deskripsi_ebook}}</textarea>
